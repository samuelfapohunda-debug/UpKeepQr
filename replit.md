...
- **Email Notifications & QR Scan Tracking**: Integrates Resend for notifications and automatically tracks QR code scans.
...
- **Unified Notification System**: Supports preference-based routing (email/SMS) via Twilio and Resend, with TCPA compliance.
- **Stripe Webhook Integration**: Two webhook endpoints: `/api/webhook/stripe` for magnet one-time orders, `/api/stripe/subscription-webhook` for subscription lifecycle. Subscription checkout generates QR codes, creates orders, and sends welcome + admin emails. Both registered in `server/src/routes/index.ts`. Extended with subscription lifecycle webhooks (7 event types) with idempotency protection via `subscription_events` table.
- **SaaS Subscription & Trial System**: Full Stripe subscription billing with 30-day trials, monthly/annual plans, 3-day grace period for failed payments, and automated lifecycle emails. Trial abuse prevention uses 4-layer defense: canonical email (Gmail dot/plus normalization), email variations check, IP address limits (3/7 days), device fingerprint limits (3/7 days). Feature gating via `feature_permissions` table. Backend: `server/routes/subscription.ts`, `server/lib/emailCanonicalization.ts`, `server/lib/trialAbuse.ts`, `server/lib/featureGating.ts`, `server/lib/subscriptionEmails.ts`. Frontend: Updated `Pricing.tsx` (monthly/annual toggle), `BillingSettings.tsx`, `SubscriptionSuccess.tsx`, `SubscriptionBanner.tsx`. Cron: Daily at 7 AM EST for grace period expiration + trial reminders. Env vars: `STRIPE_MONTHLY_PRICE_ID`, `STRIPE_ANNUAL_PRICE_ID` (set to placeholder values, update with real Stripe Price IDs).
...
- **PWA (Progressive Web App)**: Enterprise-grade PWA with service worker (`client/public/sw.js`), web manifest (`client/public/manifest.json`), offline fallback page, and intelligent caching (cache-first for static assets, network-first for API/HTML). Features: background sync with IndexedDB offline queue and exponential backoff retry, push notification infrastructure (VAPID-ready), behavior-based install prompt (`client/src/components/InstallPWA.tsx`) with iOS detection, notification permission component with rate limiting (3/day, 1/task, 4hr cooldown). Icons at `client/public/icons/` (placeholder - replace with branded PNGs). Build version injected via Vite `define`. SW registered in `client/src/main.tsx` with 30-min update checks.
