...
- **Household Creation Implementation** (Nov 23, 2025): Fully functional PostgreSQL-based household creation system. Admin submissions via `/setup/new` now create actual database records in the `households` table. Features include: conditional token validation (required for customers, optional for admins), proper Drizzle ORM `.where()` clause handling to avoid undefined errors, centralized field mapping, and audit trail tracking. Frontend uses correct JWT token key (`upkeepqr_admin_token`) for authentication headers. **Critical Fix**: Authorization header now properly included in API requests after fixing token storage key mismatch between login (`upkeepqr_admin_token`) and API request function.
