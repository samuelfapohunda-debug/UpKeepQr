...
- **Stripe Webhook Integration**: Processes `checkout.session.completed` events for order creation. Extended with subscription lifecycle webhooks (7 event types) with idempotency protection via `subscription_events` table.
