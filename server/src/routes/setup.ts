...
router.post("/activate", async (req: Request, res: Response) => {
  try {
    ...
    // STEP 5.5: GENERATE AI MAINTENANCE SCHEDULE (awaited before response — ensures tasks exist when user hits dashboard)
    try {
      ...
      console.log(`✅ AI generated ${aiTasks.length} maintenance tasks`);
    } catch (aiError) {
